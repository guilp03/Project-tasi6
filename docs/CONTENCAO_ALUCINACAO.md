...
- Se **nenhum** gap for ancorado → o status vira `"Inconclusiva"`, a criticidade vira `"Alta"` (conservadora) e o relatório mostra:

> `Análise inconclusiva: gaps gerados pela LLM não puderam ser ancorados nos artefatos do PR — revisão humana recomendada.`

**Por quê:** corta a classe de alucinação mais comum — inventar gaps que não se referem a nada no PR. A mensagem de "Inconclusiva" garante que o PMO não confunda "LLM disse OK" com "ferramenta não conseguiu confiar". A decisão seria `Crítica` se houvesse prova de risco de segurança; como não há, escolhe-se `"Alta"` conservadora, exigindo intervenção humana sem bloquear automaticamente tipos de PR sem perfil de segurança.
